...
from sqlalchemy import (
    String, Numeric, Enum, ForeignKey, DateTime,
    UniqueConstraint, func, Integer, BigInteger
)
...
class Wallet(Base):
    ...
    cash_balance: Mapped[int]      = mapped_column(BigInteger, nullable=False, default=0)
    created_at:   Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at:   Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class Holding(Base):
    ...
    avg_buy_price: Mapped[int]      = mapped_column(BigInteger, nullable=False, default=0)
    updated_at:    Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class Order(Base):
    ...
    price_at_execution: Mapped[int]         = mapped_column(BigInteger, nullable=False)
